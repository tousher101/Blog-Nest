...
function ShowPostAdmin() {
    ...
    const getallPost = async()=>{
        const token = sessionStorage.getItem('auth-token');
        const response= await fetch(`${baseApi}/api/admin/getalladminshow/${id}`,{
            method: 'GET',
            headers:  {
                'Authorization':`Bearer ${token}`
            } 
    ...
    const token = sessionStorage.getItem('auth-token');
    const response = await fetch(`${baseApi}/api/userintraction/deletecomment/${commentId}`,{
        method:'DELETE',
        headers:{
            'Authorization':`Bearer ${token}` 
        }
    ...
    const handleCancel = ()=>{
        navigator('/home/admindashboard')

    }
  return (
    <>
    {msg&&<Alert message={msg} type={type} onClose={()=>{setMsg('')}}/>}
    <div className='max-w-[1380px] mx-auto   font-[Roboto] overflow-hidden'>
        <div className='lg:flex grid grid-cols-1'>
        <div className='lg:flex-[60%] grid'>
        ...
        <div className='lg:flex-[40%] grid  mt-[20px] bg-[#E0E4EB] rounded-2xl p-[15px] mx-[10px]'>
        ...
        <div className='bg-amber-300 mx-[10px] rounded-2xl p-[15px]'>
            ...
            {getComment?.map((com)=>(
                ...
                <div className='ml-[10px] '>
                    <p className='lg:text-xl text-sm font-semibold '>{com.user.name} <span className='lg:text-sm text-xs text-gray-500'>@{com.user.role}</span><button onClick={()=>{follow(com.user.id)}}  className=' h-[30px] w-[70px] font-normal text-sm ml-[10px] rounded-xl cursor-pointer bg-blue-500 text-white'>Follow</button></p>
                </div>
                </div>
                <div className='mt-[10px]'><p className='text-md'>{com.comment}</p></div>
                <div className='flex mt-[10px] items-center'>
                    <p className='text-sm'>{new Date(com.createdAt).toDateString()}</p>
                    <button onClick={()=>{deleteCommentByAdmin(com.id)}} className=' cursor-pointer text-white text-sm ml-[50px]  w-[60px] h-[30px] rounded-xl bg-red-600'>Delete</button>
                </div>
            </div>
        ...
        <div className='flex justify-center'>
            <button onClick={handleCancel} className=' h-[45px] w-[150px] my-[20px] bg-blue-500 rounded-2xl text-xl cursor-pointer'>Cancle</button>
        </div>
