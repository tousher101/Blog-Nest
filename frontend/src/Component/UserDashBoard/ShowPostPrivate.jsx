...
function ShowPostPrivate() {
 ...
 const handleCommentNext=()=>{
    ...
    const getallPost = async()=>{
        const token = sessionStorage.getItem('auth-token');
        const response= await fetch(`${baseApi}/api/user/showprivatepost/${id}`,{
            method: 'GET',
            headers:  {
                'Authorization':`Bearer ${token}`
            } 
                ...
                <div className='bg-amber-300 mx-[10px] rounded-2xl p-[15px]'>
                    ...
                    {getComment?.map((com)=>(
                        ...
                        <div className='flex mt-[10px] items-center'>
                            <p className='lg:text-sm text-[10px]'>{new Date(com.createdAt).toDateString()}</p>
                        </div>
                    </div>
...
export default ShowPostPrivate
